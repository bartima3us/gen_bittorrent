...
%%% @end
%%% Created : 02. Aug 2019 14.22
%%%-------------------------------------------------------------------
-module(gen_bittorrent_packet_tests).
-author("bartimaeus").
...
parse_test_() ->
    ...
    [
        ...
        ?_assertEqual(
            ...
            gen_bittorrent_packet:parse_bitfield(<<16#d4, 16#f3>>)
        ),
        ?_assertEqual(
            [
                {0, true}, {1, true}, {2, false}, {3, true}, {4, false}, {5, true}, {6, false}, {7, false},
            ...
            gen_bittorrent_packet:parse_bitfield([212, 243])
        ),
        %
        % Happy day each message scenario
        ?_assertEqual(
            {ok, [{handshake, true}], undefined}, gen_bittorrent_packet:identify(Handshake1)
        ),
        ?_assertEqual(
            {ok, [{keep_alive, true}], undefined}, gen_bittorrent_packet:identify(KeepAlive1)
        ),
        ?_assertEqual(
            {ok, [{choke, true}], undefined}, gen_bittorrent_packet:identify(Choke1)
        ),
        ?_assertEqual(
            {ok, [{unchoke, true}], undefined}, gen_bittorrent_packet:identify(Unchoke1)
        ),
        ?_assertEqual(
            {ok, [{interested, true}], undefined}, gen_bittorrent_packet:identify(Interested1)
        ),
        ?_assertEqual(
            {ok, [{not_interested, true}], undefined}, gen_bittorrent_packet:identify(NotInterested1)
        ),
        ?_assertEqual(
            {ok, [{piece, PieceRecord}], undefined}, gen_bittorrent_packet:identify(Piece1)
        ),
        ?_assertEqual(
            {ok, [{have, 3}], undefined}, gen_bittorrent_packet:identify(Have1)
        ),
        ?_assertEqual(
            {ok, [{bitfield, BitFieldRecord}], undefined}, gen_bittorrent_packet:identify(BitField)
        ),
        %
        ...
        ?_assertEqual(
            ...
            gen_bittorrent_packet:parse(Data1, undefined)
        ),
        ?_assertEqual(
            {ok,
                [
            ...
            gen_bittorrent_packet:parse(Data2, undefined)
        ),
        %
        % Rainy day scenario:
        ...
        ?_assertEqual(
            ...
            gen_bittorrent_packet:parse(RainyData1, undefined)
        ),
        ?_assertEqual(
            {ok, [], Rest2},
            gen_bittorrent_packet:parse(PartBitfieldPayload2, Rest1)
        ),
        ?_assertEqual(
            {ok, [{bitfield, BitFieldRecord}], undefined},
            gen_bittorrent_packet:parse(PartBitfieldPayload3, Rest2)
        ),
        ?_assertEqual(
            {ok, [], <<0>>},
            gen_bittorrent_packet:parse(PartPiecePayload1, undefined)
        ),
        ?_assertEqual(
            {ok, [], <<0, 0, 0, 92>>},
            gen_bittorrent_packet:parse(PartPiecePayload2, <<0>>)
        ),
        ?_assertEqual(
            {ok, [{piece, PieceRecord}], undefined},
            gen_bittorrent_packet:parse(PartPiecePayload3, <<0, 0, 0, 92>>)
        )
    ].